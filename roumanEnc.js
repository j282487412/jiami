...
$(() => {
    $('.jiantou').click((e) => {
        if ($(e.target).hasClass('icon-zhankai-')) {
            $(e.target).removeClass('icon-zhankai-')
            $(e.target).addClass('icon-shouqi-')
            $(e.target).css({ left: '80%' })
            $('.mulu').show()
        } else {
            $(e.target).removeClass('icon-shouqi-')
            $(e.target).addClass('icon-zhankai-')
            $(e.target).css({ left: '0' })
            $('.mulu').hide()
        }
    })
    const stPage = localStorage.getItem(stKey) || 0;
    loadChapt(parseInt(stPage))
    $('.mulu div').click((e) => {
        $('.shitu').html('')
        loadChapt($('.mulu div').index(e.target) - 1)
    })
    $('.shitu').scroll((e) => {
        if (e.target.offsetHeight * 4 + e.target.scrollTop > e.target.scrollHeight) {
            const nextPage = $('.nextPage').attr('i');
            if (nextPage) {
                $('.nextPage').remove()
                loadChapt(parseInt(nextPage));
            }
        }
    })
})

function loadChapt(i = 0) {
    localStorage.setItem(stKey, i)
    var href = $($('.mulu div')[i + 1]).attr('href');
    const title = $($('.mulu div')[i + 1]).text();
    if (href) {
        $.ajax({
            url: href, success: (msg) => {
                $('.shitu').append('<h5>' + title + '</h5>')
                var sss = $(msg).filter('script')
                ddd = sss[sss.length - 2];
                var self = { __next_f: [] };
                eval(ddd.innerText);
                var imgs = JSON.parse(self.__next_f[self.__next_f.length - 1][1].substr(2))[3].children[3][3].children.map(c => c[3].children[3].imageUrl);
                saveHtml(imgs)
            }
        })
    } else {
        $('.shitu').append('<div class="ending">结束或完结了</div>')
    }
}
function saveHtml(imgs, i) {
    var html = [];
    for (let s of imgs) {
        html.push('<img src="' + s + '" onload="loadImg(this)">');
    }
    $('.shitu').append(html)
    $('.shitu').append('<div class="nextPage" i="' + (i + 1) + '"></div>')
}

function loadImg(obj) {
    const a = $(obj).attr('src')
    if (a.includes("sr:1")) {
        var n = obj.naturalWidth, o = obj.naturalHeight;
        var t = document.createElement("canvas");
        let r = t.getContext("2d", { alpha: !1 });
        t.width = n, t.height = o;
        let c = a.split("/")
            , s = N(atob(c[c.length - 1].replace(".jpg", "")));
        for (var u = slice(y(s, "hex"), -1)[0] % 10 + 5, d = o % u, f = 0; f < u; f++) {
            var m = Math.floor(o / u)
                , b = m * f
                , p = o - m * (f + 1) - d;
            0 == f ? m += d : b += d,
                r.drawImage(obj, 0, p, n, m, 0, b, n, m)
        }
        $(t).insertAfter(obj)
        $(obj).hide()
    }
